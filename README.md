...
- Native IPv4/IPv6 routes and metrics, interface indexes/MTU/DNS, network profiles, NIC counters, bounded structured logs, snapshot export, and log export provide the first Step 2 increments.
- Next: finish DPI checks, then add network bindings and the remaining TCP, offload, QoS, Winsock, monitoring, report, history, and comparison surfaces.
