...
>
> SockTuner's NIC list is **hardware-verified**: `WindowsNdisInventory` enumerates only what
> the driver actually advertises. [`NicKeywordCatalog`](../src/SockTuner/Services/NicKeywordCatalog.cs)
> then *annotates* that list — it never extends it. It carries 23 curated entries distilled from
> §C and the "safe-ish levers" list below (6 driver-advertised, 10 situational, 7 rejected);
> every other keyword is classified by the Microsoft `*`-prefix rule, so an unlisted `*` keyword
> reads as driver-advertised at its own range and an unlisted vendor keyword reads as
> uncharacterised. The ~90 properties below are the *superset the Zenit tool writes blindly by
