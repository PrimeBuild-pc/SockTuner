...
A measurement now carries the cadence it should be read against. A game server does not process input continuously: it advances the world in fixed steps, and that rate decides what a given number costs. Five milliseconds of jitter is a rounding error against a 50 ms update and is most of the budget against a 7.8 ms one, so the jitter limits are derived from the selected tick rate rather than chosen — half a tick is comfortable, one tick is the edge — and both are capped at 15 ms and 30 ms, because the tick arithmetic alone would call 90 ms of jitter fine on a 10 Hz game, which is true of the tick and useless to a person. The latency limits tighten with the tick rate and are labelled as the judgement they are. The loss limits are neither derived nor scaled: a dropped packet is a dropped input at every tick rate. The verdict is the worst of the three, never their average, and the metric that decided it is named so it can be checked against the table.

`GameProfiles` states where each tick rate came from — developer-stated, community-reported, or a band that deliberately does not claim a title. A rate matched from an imported capture never snaps to a generic band, because that would put a game's name on a capture that never carried one; an unmatched rate is added as its own entry instead. The tick rate is what SockTuner judges against and deliberately not what it sends at: a serial ICMP probe cannot reach 128 packets a second against a real path, and sending at a game's rate to somebody else's server uninvited is not this app's business. What the game does set is the size of each probe, since a live game packet is not the 32 bytes a default ping carries.

Two measurement details follow from that. Jitter is now also reported over fixed one-second windows — the standard deviation inside each second, averaged — because the existing consecutive-difference figure moves with how fast the profile sent, and a threshold derived from a tick rate needs a figure that does not. It is blank rather than fabricated when no second collected two replies. And a monitoring run reports availability as a share of *time*, taken from the worst target rather than summed across them: a line that answers 99.98 % of probes but goes silent for four seconds twice an evening is the exact fault a game notices and a packet average hides. Its resolution is the probe interval, which the report says rather than implies — as it says that Windows reports an ICMP round trip in whole milliseconds, which at 128 Hz is a noticeable share of the budget being judged.

Two capabilities from the same measurement sites are declined on the record. A percentile against other people's tests needs a stored corpus and therefore a backend, which this project does not have and does not want. A "your server is 542 km away, so 5.4 ms of your ping is light in fibre" line needs the endpoint geolocated by a third-party service, and what it concludes — that the rest is equipment — the bottleneck locator already establishes from the path itself, without the dependency. Blocking a game's datacenters at the firewall, as FAST GeoFence does, is declined for a different reason: it is a per-title change that buys a better server with failed queues, and it is not a network fault being fixed.
